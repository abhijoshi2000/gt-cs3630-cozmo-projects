...
class GUIWindow():
    ...
    def drawGrid(self):
        for y in range(1, self.grid.height):
            self.canvas.create_line(
                0, y * self.grid.scale, int(self.canvas.cget("width")) - 1, y * self.grid.scale)
        for x in range(1, self.grid.width):
            self.canvas.create_line(
                x * self.grid.scale, 0, x * self.grid.scale, int(self.canvas.cget("height")) - 1)
    ...
    def drawMarkers(self):
        for marker in self.markers:
            marker_x, marker_y, marker_h = parse_marker_info(
            ...
            self.colorRectangle((marker_x+c1x, marker_y+c1y),
                                (marker_x+c2x, marker_y+c2y), '#00FFFF')

    def drawCameraImage(self):
        if self.camera_image:
            self.camera_canvas.image = ImageTk.PhotoImage(self.camera_image)
            self.camera_canvas.create_image(
                0, 0, anchor=NW, image=self.camera_canvas.image)
    ...
    def _show_particles(self, particles):
        plot_cnt = PARTICLE_MAX_SHOW if len(
            particles) > PARTICLE_MAX_SHOW else len(particles)
    ...
    def _show_robot(self, robot):
        ...
        self.colorLine(coord, (coord[0]+fov_lx, coord[1]+fov_ly),
                       color='#222222', linewidth=2, dashed=True)
        self.colorLine(coord, (coord[0]+fov_rx, coord[1]+fov_ry),
                       color='#222222', linewidth=2, dashed=True)
    ...
    def colorCell(self, location, color):
        coords = (location[0]*self.grid.scale,
                  (self.height-location[1]-1)*self.grid.scale)
        self.canvas.create_rectangle(
            coords[0], coords[1], coords[0] + self.grid.scale, coords[1] + self.grid.scale, fill=color)

    def colorRectangle(self, corner1, corner2, color):
        coords1 = (corner1[0]*self.grid.scale,
                   (self.height-corner1[1])*self.grid.scale)
        coords2 = (corner2[0]*self.grid.scale,
                   (self.height-corner2[1])*self.grid.scale)
        self.canvas.create_rectangle(
            coords1[0], coords1[1], coords2[0], coords2[1], fill=color)

    def colorCircle(self, location, color, dot_size=5):
        x0, y0 = location[0]*self.grid.scale - \
            dot_size, (self.height-location[1])*self.grid.scale - dot_size
        x1, y1 = location[0]*self.grid.scale + \
            dot_size, (self.height-location[1])*self.grid.scale + dot_size
        # print(x0,y0,x1,y1)
    ...
    def colorTriangle(self, location, heading_deg, color, tri_size):
        ...
        hrot = (hx + location[0]*self.grid.scale, -hy +
                (self.height-location[1])*self.grid.scale)
        lrot = (lx + location[0]*self.grid.scale, -ly +
                (self.height-location[1])*self.grid.scale)
        rrot = (rx + location[0]*self.grid.scale, -ry +
                (self.height-location[1])*self.grid.scale)
    ...
    def show_camera_image(self, image):
        self.lock.acquire()
        self.camera_image = image.resize(
            (self.grid.width * self.grid.scale, self.grid.height * self.grid.scale))
        self.lock.release()
    ...
    def update(self):
        ...
        self._show_mean(self.mean_x, self.mean_y,
                        self.mean_heading, self.mean_confident)
        if self.robot != None:
    ...
    def start(self):
        master = Tk()
        master.wm_title(
            "Particle Filter: Grey/Green - estimated, Red - ground truth")

        self.canvas = Canvas(master, width=self.grid.width * self.grid.scale,
