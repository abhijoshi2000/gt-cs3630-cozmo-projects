...
class Visualizer():
    ...
    def draw_color_poly(self, coords, color, bg=False, tags=''):
        """Draw a colored polygon at a given coord
            ...
            tags -- tags to apply to square, list of strings or string
        """
        coords_flipped = [(coord[0], (self.cmap.height - 1 - coord[1]))
                          for coord in coords]
        rect = self.canvas.create_polygon(
            coords_flipped, fill=color, tags=tags)
        if bg:
            self.canvas.tag_lower(rect)
    ...
    def draw_start(self):
        ...
        if self.cmap._start != None:
            self.draw_color_circle(
                self.cmap._start, '#00DD00', size=15, bg=True, tags='start')
    ...
    def draw_goals(self):
        ...
        for goal in self.cmap._goals:
            self.draw_color_circle(
                goal, '#0000DD', size=15, bg=True, tags='goal')
    ...
    def draw_node_path(self):
        ...
        for node_path in self.cmap._node_paths:
            self.draw_edge(node_path[0], node_path[1],
                           color='#DD0000', width=2, tags='node_paths')
    ...
    def draw_solution(self):
        ...
        for p in range(len(path)-1):
            self.draw_edge(path[p], path[p+1],
                           color='#DDDD00', width=5, tags='smoothed')
    ...
    def start(self):
        ...
        self.canvas = Canvas(master, width=self.cmap.width,
                             height=self.cmap.height, bd=0, bg='#FFFFFF')
        self.canvas.pack()
