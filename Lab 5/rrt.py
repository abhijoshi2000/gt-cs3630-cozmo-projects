...
def step_from_to(node0, node1, limit=75):
    ...
    if get_dist(node0, node1) < limit:
        ...
        x_coords = node1.x - node0.x
        y_coords = node1.y - node0.y
        vect_angle = np.arctan2(y_coords, x_coords)
        target_x = node0.x + (limit * np.cos(vect_angle))
        target_y = node0.y + (limit * np.sin(vect_angle))
...
def node_generator(cmap):
    ...
    if prob > 0.95:
        while (rand_node is None):
            ...
            if cmap.is_inside_obstacles(rand_node) or not cmap.is_inbound(rand_node):
                rand_node = None
            else:
                return rand_node
    else:
...
def RRT(cmap, start):
    ...
    while (cmap.get_num_nodes() < MAX_NODES):
        ...
        nearest_node = None
        rrt_nodes = cmap.get_nodes()
        min_dist = math.inf
        for node in rrt_nodes:
            curr_node_dist = get_dist(node, rand_node)
            if curr_node_dist < min_dist:
                nearest_node = node
                min_dist = curr_node_dist
...
def get_global_node(local_angle, local_origin, node):
    ...
    transformed_matrix = np.dot(transform, node_pos)
    new_node_x = (transformed_matrix[0, 0])
    new_node_y = (transformed_matrix[1, 0])
...
if __name__ == '__main__':
    ...
    robotFlag = False
    # reads input whether we are running the robot version or not
