    ...
    def apply(self, image, scale):
        ...
        def print_line(text_line):
            text = cozmo.annotate.ImageText(
                text_line, position=cozmo.annotate.TOP_LEFT, outline_color='black', color='lightblue')
        ...
        print_line('Pose: Rot quat = <%.1f, %.1f, %.1f, %.1f>' %
        ...
        if robot.device_accel_raw is not None:
            print_line('Device Acc Raw: <%.2f, %.2f, %.2f>' %
                       robot.device_accel_raw.x_y_z)
        if robot.device_accel_user is not None:
            print_line('Device Acc User: <%.2f, %.2f, %.2f>' %
                       robot.device_accel_user.x_y_z)
...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    # number of images to take of each type of object
    num_images_per_type = int(myargs[0])
    ...
    robot.drive_straight(distance_mm(
        1000), speed_mmps(1000)).wait_for_completed()
